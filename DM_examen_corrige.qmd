...
18. Formulez une interprétation en croisant le tri croisé du sentiment d'appartenance et de la position sociale avec le graphique représentant la distribution du niveau maximum d'étude atteint par les parents selon la position sociale

```{r}
wtd.table(pisa$etud_par, weights = pisa$poids) %>%
  proportions()
```
