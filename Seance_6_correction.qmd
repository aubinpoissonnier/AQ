...
Une visualisation graphique par des modèles localisés, qu'on ne traite pas dans le cours.
...
```{r}
mod <- lm(score_compecr ~ pos_soc, data = pisa, subset = immig2 == "Oui")
library(segmented)

my.seg <- segmented(mod,
                    seg.Z = ~ pos_soc,
                    psi = 0)
...
my.model <- data.frame(pos_soc = mod$model$pos_soc, score = my.fitted)

ggplot(my.model) +
  geom_line(aes(x = pos_soc,
                y = score))

table(is.na(pisa$score_compecr))
